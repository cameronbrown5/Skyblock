    ...
    public void openGUI(CommandSender sender, String category, @Optional OnlinePlayer target, @Optional OnlinePlayer open) {
        ...
        if(sender instanceof Player player)
            if(target != null && open != null)
                questMenu.getGui(target.getPlayer(), category.toLowerCase(), "Builder Quests").show(open.getPlayer());
            else if(target != null)
                questMenu.getGui(target.getPlayer(), category.toLowerCase(), "Builder Quests").show(player);
            else
                questMenu.getGui(player, category.toLowerCase(), "Builder Quests").show(player);
        else {
            if(target != null && open != null)
                questMenu.getGui(target.getPlayer(), category.toLowerCase(), "Builder Quests").show(open.getPlayer());
            else if(target != null)
                questMenu.getGui(target.getPlayer(), category.toLowerCase(), "Builder Quests").show(target.getPlayer());
            else {
